...
function loadBookInfo() {
  $("#thumbnail").attr(
    "src",
    "https://ndlsearch.ndl.go.jp/thumbnail/" + arg.isbn + ".jpg"
  );
  let url =
  ...
  $.ajax({
    ...
    success: function (data) {
      $("item", data).each(function () {
        ...
        $("#authors").text(
          $("dc\\:creator", this).text() +
            "｜読み:" +
            $("dcndl\\:creatorTranscription", this).text()
        );
        $("#large-authors").text($("dc\\:creator", this).text());
        $("#ISBN13").text(
          $("dc\\:identifier[xsi\\:type=dcndl\\:ISBN]", this)
            .text()
            .slice(0, 17)
        );
        if ($("dc\\:subject[xsi\\:type=dcndl\\:NDC10]", this).text() != "") {
          $("#ndc10").text(
            $("dc\\:subject[xsi\\:type=dcndl\\:NDC10]", this).text()
          );
        } else if (
          $("dc\\:subject[xsi\\:type=dcndl\\:NDC9]", this).text() != ""
        ) {
          $("#ndc10").text(
            $("dc\\:subject[xsi\\:type=dcndl\\:NDC9]", this).text()
          );
        ...
        $("#pageCount").text(
          $("dc\\:extent", this).text().slice(0, -1) + "ページ"
        );
