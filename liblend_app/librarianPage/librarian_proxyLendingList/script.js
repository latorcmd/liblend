...
function proxylogin(request) {
    $.ajax({
        ...
        type: 'POST',
        dataType: 'json',
        data: '{"requestType":"proxylogin","userCode":' + $('#userCode').val() + ',"password":""}',
...
function searchBook(num) {
    ...
    if (defaultAPIEngine == 'google') {
        var url = "https://www.googleapis.com/books/v1/volumes?q=isbn:" + isbn + "&key=" + googleAPIKey;
        ...
        $.getJSON(url, function(data) {
            if (!data.totalItems) {
                ...
                $('#returningDate_' + num).val(year + '/' + month + '/' + day + '(' + weekDay + ')');
        ...
        var url = "https://ndlsearch.ndl.go.jp/api/opensearch?dpid=iss-ndl-opac%20iss-ndl-opac-inprocess%20zassaku%20zassaku-online%20ndl-dl-open%20jpro&isbn=" + isbn;
